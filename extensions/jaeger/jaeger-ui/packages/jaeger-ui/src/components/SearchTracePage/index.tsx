...
export class SearchTracePageImpl extends Component<SearchTracePageImplProps, ISearchTracePageImplState> {
  ...
  componentDidUpdate(prevProps: SearchTracePageImplProps) {
    const { urlQueryParams, queryOfResults, searchTraces, isHomepage } = this.props;

    // If URL params changed and they're different from current results, trigger a new search
    if (
      !isHomepage &&
      urlQueryParams &&
      prevProps.urlQueryParams !== urlQueryParams &&
      (!queryOfResults || !isSameQuery(urlQueryParams as any, queryOfResults as any))
    ) {
      searchTraces(urlQueryParams);
    }
...
const stateServicesXformer = memoizeOne((stateServices: ReduxState['services']) => {
  ...
  // Don't block form rendering if operations are still loading - the form can handle that internally
  const services =
    serviceList &&
